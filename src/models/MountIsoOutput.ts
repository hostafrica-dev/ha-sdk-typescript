/* tslint:disable */
/* eslint-disable */
/**
 * HostAfricaApi
 * HostAfrica API - Manages VPS instances and operations
 *
 * The version of the OpenAPI document: 2026-01-01
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { VpsSimpleActionResponseData } from './VpsSimpleActionResponseData';
import {
    VpsSimpleActionResponseDataFromJSON,
    VpsSimpleActionResponseDataFromJSONTyped,
    VpsSimpleActionResponseDataToJSON,
    VpsSimpleActionResponseDataToJSONTyped,
} from './VpsSimpleActionResponseData';
import type { OperationStatus } from './OperationStatus';
import {
    OperationStatusFromJSON,
    OperationStatusFromJSONTyped,
    OperationStatusToJSON,
    OperationStatusToJSONTyped,
} from './OperationStatus';

/**
 * 
 * @export
 * @interface MountIsoOutput
 */
export interface MountIsoOutput {
    /**
     * 
     * @type {OperationStatus}
     * @memberof MountIsoOutput
     */
    status: OperationStatus;
    /**
     * 
     * @type {VpsSimpleActionResponseData}
     * @memberof MountIsoOutput
     */
    data: VpsSimpleActionResponseData;
}



/**
 * Check if a given object implements the MountIsoOutput interface.
 */
export function instanceOfMountIsoOutput(value: object): value is MountIsoOutput {
    if (!('status' in value) || value['status'] === undefined) return false;
    if (!('data' in value) || value['data'] === undefined) return false;
    return true;
}

export function MountIsoOutputFromJSON(json: any): MountIsoOutput {
    return MountIsoOutputFromJSONTyped(json, false);
}

export function MountIsoOutputFromJSONTyped(json: any, ignoreDiscriminator: boolean): MountIsoOutput {
    if (json == null) {
        return json;
    }
    return {
        
        'status': OperationStatusFromJSON(json['status']),
        'data': VpsSimpleActionResponseDataFromJSON(json['data']),
    };
}

export function MountIsoOutputToJSON(json: any): MountIsoOutput {
    return MountIsoOutputToJSONTyped(json, false);
}

export function MountIsoOutputToJSONTyped(value?: MountIsoOutput | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'status': OperationStatusToJSON(value['status']),
        'data': VpsSimpleActionResponseDataToJSON(value['data']),
    };
}

